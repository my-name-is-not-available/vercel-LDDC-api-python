...
# 移除对PySide6的依赖，定义模拟类以保持兼容性
class MockQMessageLogContext:
    def __init__(self, file='?', line=0, function='?', category='?'):
        self.file = file
        self.line = line
...
# 替换原始的PySide6类
QLoggingCategory = MockQLoggingCategory
QMessageLogContext = MockQMessageLogContext
QtMsgType = MockQtMsgType

# 空实现qInstallMessageHandler函数
...
# 简化的qt_message_handler函数，不再依赖PySide6
def qt_message_handler(mode: int, context: QMessageLogContext, message: str) -> None:
    # 这里我们直接使用Python标准日志，不再区分Qt消息类型
